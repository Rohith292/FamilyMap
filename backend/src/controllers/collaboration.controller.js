...
export const sendInvitation = async (req, res) => {
    ...
    try {
        ...
        if (collaboration.status !== 'revoked' && collaboration.status !== 'declined') {
            return res.status(400).json({ message: "Cannot delete an active or pending collaboration record. Please revoke access first." });
        }
