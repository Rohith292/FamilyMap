...
export const createAlbum = async (req, res) => {
    try {
        const { name, description, sharedWithGroups } = req.body; // <-- DESTRUCTURING sharedWithGroups
        ...
        // --- NEW: Authorization Check for Group Creation ---
        if (sharedWithGroups && sharedWithGroups.length > 0) {
            // Find the group(s) to verify the user is a member of all of them
        ...
        const newAlbum = new Album({
            ...
            description,
            owner,
            sharedWithGroups: sharedWithGroups || [] // <-- ADD THIS FIELD
        });
        console.log("Album to be saved. sharedWithGroups:", newAlbum.sharedWithGroups); // <-- ADD THIS LINE
        const savedAlbum = await newAlbum.save(); // Capture the saved result
        console.log("Album saved successfully:", savedAlbum); // <-- ADD THIS LOG
...
export const getAlbumsByFamilyGroup = async (req, res) => {
    try {
        ...
        // Verify that the user is a member of the family group
        ...
        // --- Step 2: Fetch Albums ---
        // Find all albums where the familyGroup's ID is in the sharedWithGroups array
        const albums = await Album.find({
            sharedWithGroups: { $in: [groupId] }
        }).sort({ createdAt: -1 });
        console.log("Albums found for group:", albums);
        res.status(200).json({ albums });
