 ...
 export const getCollaborations = async (req, res) => {
            try {
                ...
                const asOwner = await Collaboration.find({ familyMapOwnerId: userId })
                    .populate('collaboratorId', 'username email') // Populate collaborator details
                    .populate('familyMapOwnerId', 'username email') // Populate owner details (useful for sharedWithMe)
                ...
                const asCollaborator = await Collaboration.find({ collaboratorId: userId })
                    .populate('familyMapOwnerId', 'username email') // Populate owner details
                    .populate('collaboratorId', 'username email') // Populate collaborator details (useful for owned)
                    .sort({ createdAt: -1 }); // Newest first
...
export const getFamilyMemberByGroup = asyncHandler(async (req, res) => {
    try {
        ...
        // 1. First, check if the current user is a member of the requested group.
        const familyGroup = await FamilyGroup.findById(groupId);
        if (!familyGroup) {
            return res.status(404).json({ message: "Family group not found" });
        }

        const isMember = familyGroup.members.some(memberId => memberId.toString() === req.user._id.toString());
        if (!isMember) {
            return res.status(403).json({ message: "Not authorized to view the family members of this group" });
        }

        // 2. Fetch all members explicitly belonging to this group.
        // We need to retrieve their parentId and partners fields to traverse the tree.
        ...
        // 3. Initialize a Set to store all unique member IDs in the tree.
        // Sets are faster for checking for existence and prevent duplicates.
        const allMemberIdsInTree = new Set(membersInGroup.map(m => m._id.toString()));
        let newIdsAdded = true;

        // 4. Traverse the family tree from the initial group members.
        // Keep looping as long as new members are being added to the tree.
        while (newIdsAdded) {
            newIdsAdded = false;

            // Get a list of member IDs that we have found so far.
            const currentTreeIds = Array.from(allMemberIdsInTree);
            
            // Find all family members that are parents, children, or partners of anyone in our current tree.
            // We use the $or operator to search for connections in three ways.
            const newlyFoundMembers = await FamilyMember.find({
                $or: [
                    { parentId: { $in: currentTreeIds } }, // Find children of current tree members
                    { partners: { $in: currentTreeIds } }  // Find partners of current tree members
                ],
                // Exclude any members we've already found to avoid duplicates and infinite loops.
            ...
            if (newlyFoundMembers.length > 0) {
                newlyFoundMembers.forEach(member => {
                    if (!allMemberIdsInTree.has(member._id.toString())) {
                        ...
                        newIdsAdded = true; // A new member was added, so we should loop again.
                    }
                });
            }
        }
        
        // 5. After the loop completes, fetch the complete data for all members in our Set.
        const familyMembers = await FamilyMember.find({
            _id: { $in: Array.from(allMemberIdsInTree) }
        })
        .populate('createdBy', 'fullName email')
        .populate('parentId', 'name')
        // Note: We don't need to populate 'children' since it's not a field in your schema.
        // The frontend will build the connections itself based on the parentId field.
